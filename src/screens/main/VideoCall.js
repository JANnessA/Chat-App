...
import {AgoraAppId, SocketEvent} from '../../configs';
import Context from '../../helpers/context';
import {endCalling, declineCall} from '../../helpers/network';
...
class VideoCall extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      conversationId: this.props.route.params.conversationId, //Channel Name for the current session
      vidMute: this.props.route.params.vidMute, //State variable for Video Mute
  ...
  componentDidMount() {
    this.context.socket.on(SocketEvent.DECLINE_CALL, data => {
    ...
    RtcEngine.joinChannel(this.state.conversationId, this.state.uid); //Join Channel
    RtcEngine.enableAudio(); //Enable the audio
  }
  componentWillUnmount() {
    this.context.socket.off(SocketEvent.DECLINE_CALL);
  }
  /**
   * @name toggleAudio
  ...
  async endCall() {
    if (this.state.peerIds.length > 0) {
      await endCalling({conversationId: this.state.conversationId});
    }
    if (!this.state.isCalled) {
      await declineCall({conversationId: this.state.conversationId});
    }
    RtcEngine.destroy();
